...
echo [1/4] Preparing pinned CMake, Ninja, and LLVM-MinGW tools...
call :prepare_tools
if errorlevel 1 goto :failed

set "CMAKE=!TOOLS_ROOT!\cmake\bin\cmake.exe"

echo [2/4] Configuring the release installer...
pushd "project\installer"
...
echo [3/4] Building Motorhead and Installer.exe...
...
echo [4/4] Staging the test package...
if not exist "bin" mkdir "bin"
copy /Y "project\installer\build\bin\Installer.exe" "bin\Installer.exe" >nul
if errorlevel 1 goto :failed

echo.
echo Build complete:
echo   %CD%\bin\Installer.exe
echo.
echo Place your verified mhp30.exe beside Installer.exe, then run Installer.exe.
popd
pause
exit /b 0
...
:prepare_tools
if exist "!TOOLS_ROOT!\cmake\bin\cmake.exe" if exist "!TOOLS_ROOT!\ninja\ninja.exe" if exist "!TOOLS_ROOT!\llvm-mingw-xp\bin\i686-w64-mingw32-clang.exe" (
...
set "TOOL_ID=llvm-mingw-xp"
set "TOOL_VERSION=22.1.7"
set "TOOL_ARCHIVE=llvm-mingw-xp-22.1.7-msvcrt-x86_64.zip"
set "TOOL_URL=https://github.com/mon/llvm-mingw-xp/releases/download/llvm-mingw-xp-22.1.7/llvm-mingw-xp-22.1.7-msvcrt-x86_64.zip"
set "TOOL_HASH=042B2DF678EAB60F42BF47D0024BEC3572C25D327B19B0AA14CDA2B622EF3E1B"
set "TOOL_ROOT_DIR=llvm-mingw-xp-22.1.7-msvcrt-x86_64"
set "TOOL_SENTINEL=bin\i686-w64-mingw32-clang.exe"
call :ensure_tool
if errorlevel 1 exit /b 1
