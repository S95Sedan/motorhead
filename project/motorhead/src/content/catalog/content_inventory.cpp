...
Inventory inventory_tree(const std::filesystem::path& root) {
    ...
    for (const auto& path : paths) {
        ...
        const std::vector<std::uint8_t> header(
            prefix.begin(), std::next(prefix.begin(), static_cast<std::ptrdiff_t>(header_size)));
        InventoryFile file;
        ...
        auto canonical = file.relative_path;
        canonical.push_back('\0');
        canonical += std::to_string(file.bytes);
        canonical.push_back('\0');
        canonical += file.sha256;
        canonical.push_back('\0');
        canonical += file.identification.format_id;
        canonical.push_back('\0');
        canonical += file.identification.status;
        canonical.push_back('\n');
