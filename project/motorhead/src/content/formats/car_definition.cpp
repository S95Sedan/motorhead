...
audit_car_color_slots(const MyoData &model,
                      const std::array<CarColor, 3U> &default_colors) {
  CarColorSlotAudit result;
  for (const auto &face : model.faces) {
    const auto mask = exact_car_color_slot_mask(face.color, default_colors);
    if (mask == 0U) {
      ++result.unmatched_face_count;
      if (face.color == std::array<std::uint8_t, 3U>{0U, 0U, 0U}) {
        ++result.unmatched_black_face_count;
      }
      continue;
    }
    if ((mask & static_cast<std::uint8_t>(mask - 1U)) != 0U) {
      ++result.ambiguous_face_count;
      continue;
    }
    ...
    ++result.face_counts[*slot];
    if (face.has_texture_coordinates) {
      ++result.textured_face_counts[*slot];
    } else {
      ++result.untextured_face_counts[*slot];
    }
    if (face.primitive_type == 8U || face.primitive_type == 9U) {
      ++result.environment_face_counts[*slot];
      if (face.has_vertex_normals) {
        ++result.reflective_face_counts[*slot];
      }
    }
  }
  return result;
