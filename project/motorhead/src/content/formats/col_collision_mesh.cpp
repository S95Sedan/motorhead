...
void write_col_obj(
    const ColData& collision, const std::filesystem::path& output, const bool overwrite) {
    if (output.empty()) { throw ToolError(ExitCode::usage, "COL OBJ output path is empty"); }
    const auto polygons = reconstruct_col_polygons(collision);
    ...
    for (const auto& component : parent.relative_path()) {
        ...
        if (error == std::errc::no_such_file_or_directory) {
            ...
            if (!std::filesystem::create_directory(current, error) || error) {
                throw ToolError(ExitCode::input, "cannot create COL OBJ output directory");
            }
            status = std::filesystem::symlink_status(current, error);
        }
        if (error || std::filesystem::is_symlink(status)
            || !std::filesystem::is_directory(status)) {
            throw ToolError(ExitCode::input, "COL OBJ output parent is not a plain directory");
        }
    }
    ...
    if (exists && (std::filesystem::is_symlink(status)
        || !std::filesystem::is_regular_file(status))) {
        throw ToolError(ExitCode::input, "COL OBJ output is not a plain file");
    }
    if (exists && !overwrite) {
        throw ToolError(ExitCode::input, "COL OBJ output exists (use --overwrite)");
    }
    ...
    if (std::filesystem::exists(temporary) || (exists && std::filesystem::exists(backup))) {
        throw ToolError(ExitCode::input, "COL OBJ temporary or backup output exists");
    }
    std::ostringstream text;
    text << "# Motorhead COL collision polygon export\n" << std::setprecision(9)
         << "o collision\n";
    std::uint64_t vertex_index = 1U;
    std::uint64_t normal_index = 1U;
    for (const auto& polygon : polygons) {
        text << "# face " << polygon.face_index << " surface " << polygon.surface << '\n';
        for (const auto& vertex : polygon.vertices) {
            text << "v " << vertex[0] << ' ' << vertex[1] << ' ' << vertex[2] << '\n';
        }
        text << "vn " << polygon.normal[0] << ' ' << polygon.normal[1] << ' '
             << polygon.normal[2] << "\nf";
        for (std::size_t index = 0U; index < polygon.vertices.size(); ++index) {
            text << ' ' << vertex_index + index << "//" << normal_index;
        }
        text << '\n';
        vertex_index += polygon.vertices.size();
        ++normal_index;
    }
    {
        ...
        if (!stream) {
            ...
            throw ToolError(ExitCode::input, "cannot write COL OBJ output");
        }
    }
    ...
    if (exists) {
        ...
        if (error) {
            std::filesystem::remove(temporary, error);
            throw ToolError(ExitCode::input, "cannot preserve COL OBJ output");
        }
        backed_up = true;
    }
    ...
    if (error) {
        ...
        throw ToolError(ExitCode::input, "cannot finalize COL OBJ output");
    }
    if (backed_up) {
        std::filesystem::remove(backup, error);
        if (error) { throw ToolError(ExitCode::input, "cannot remove COL OBJ backup"); }
    }
}
