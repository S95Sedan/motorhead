...
void ensure_plain_parent(const std::filesystem::path& parent) {
    const auto absolute = std::filesystem::absolute(parent).lexically_normal();
    auto current = absolute.root_path();
    for (const auto& component : absolute.relative_path()) {
        current /= component;
        const auto status = symlink_status(current, "TBL output directory");
        if (!std::filesystem::exists(status)) {
            std::error_code error;
            if (!std::filesystem::create_directory(current, error) || error) {
                throw ToolError(ExitCode::input,
                    "cannot create TBL output directory: " + current.string());
            }
        } else if (std::filesystem::is_symlink(status)
            || !std::filesystem::is_directory(status)) {
            throw ToolError(ExitCode::input,
                "TBL output parent is not a plain directory: " + current.string());
        }
    }
}
...
std::vector<std::uint8_t> encode_tbl_transform(
    const std::span<const std::uint8_t> decoded, const std::string_view key_name) {
    auto result = std::vector<std::uint8_t>(decoded.begin(), decoded.end());
    const auto primary = expand_key(primary_key_source, primary_key_period);
    const auto name = name_bytes(key_name);
    const auto secondary = expand_key(name, name_key_period);
    ...
    for (std::size_t reverse = 0U; reverse < result.size(); ++reverse) {
        const auto index = result.size() - reverse - 1U;
        const auto plain = result[index];
        result[index] = static_cast<std::uint8_t>(plain + accumulator);
        accumulator = static_cast<std::uint8_t>(accumulator + plain);
    }

    accumulator = 0U;
    for (std::size_t index = 0U; index < result.size(); ++index) {
        const auto plain = result[index];
        std::uint32_t value = plain;
        value += accumulator;
        value += primary[index % primary.size()];
        value += secondary[index % secondary.size()];
...
TblEnvelope read_tbl(
    ...
    const auto header_size = std::min<std::size_t>(32U, result.payload.size());
    result.transformed_header.assign(result.payload.begin(),
        std::next(result.payload.begin(), static_cast<std::ptrdiff_t>(header_size)));
...
void write_tbl_payload(
    ...
    if (!envelope.decoded) {
        throw ToolError(ExitCode::format,
            "TBL payload is not decoded: " + envelope.decode_status);
    }
    if (output.empty()) { throw ToolError(ExitCode::usage, "TBL output path is empty"); }
    ...
    ensure_plain_parent(parent);
    const auto status = symlink_status(destination, "TBL output");
    const bool exists = std::filesystem::exists(status);
    if (exists) {
        if (std::filesystem::is_symlink(status)
            || !std::filesystem::is_regular_file(status)) {
            throw ToolError(ExitCode::input, "TBL output is not a plain file");
        }
        if (!overwrite) { throw ToolError(ExitCode::input, "TBL output exists (use --overwrite)"); }
    }

    auto temporary = destination;
    temporary += ".mhtool-part";
    if (std::filesystem::exists(symlink_status(temporary, "temporary TBL output"))) {
        throw ToolError(ExitCode::input, "temporary TBL output already exists");
    }
    auto backup = destination;
    backup += ".mhtool-backup";
    if (exists && std::filesystem::exists(symlink_status(backup, "TBL output backup"))) {
        throw ToolError(ExitCode::input, "TBL output backup already exists");
    }
    bool created = false;
    try {
        std::ofstream stream(temporary, std::ios::binary | std::ios::out | std::ios::trunc);
        if (!stream) { throw ToolError(ExitCode::input, "cannot create TBL output"); }
        created = true;
        stream.write(reinterpret_cast<const char*>(envelope.payload.data()),
            static_cast<std::streamsize>(envelope.payload.size()));
        stream.close();
        if (!stream) { throw ToolError(ExitCode::input, "cannot write TBL output"); }
        std::error_code error;
        ...
        if (exists) {
            ...
            if (error) {
                throw ToolError(ExitCode::input,
                    "cannot preserve existing TBL output: " + error.message());
            }
            backup_created = true;
        }
        ...
        if (error) {
            ...
            if (backup_created) {
                ...
                if (rollback_error) {
                    rollback_message = "; existing output remains at " + backup.string()
                        + " because rollback failed: " + rollback_error.message();
                }
            }
            throw ToolError(ExitCode::input,
                "cannot finalize TBL output: " + error.message() + rollback_message);
        }
        created = false;
        if (backup_created) {
