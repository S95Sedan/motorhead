...
float4 shade_scene(VertexOutput input, bool additive) {
  const float4 texel = scene_texture.Sample(scene_sampler,
...
bool D3D11Renderer::initialize(SDL_Renderer *renderer) {
...
bool D3D11Renderer::available() const noexcept {
  return impl_->device && impl_->context;
}

SDL_Texture *D3D11Renderer::render(SDL_Renderer *renderer,
...
double D3D11Renderer::last_render_ms() const noexcept {
