...
static bool D3D_CreateRenderer(SDL_Renderer *renderer, SDL_Window *window, SDL_PropertiesID create_props)
{
    ...
    // SDL's display-to-D3D9 adapter mapping can return an unavailable index
    // while a newly-created window is moving between modern heterogeneous
    // adapters. Validate it before GetDeviceCaps instead of rejecting the
    // native D3D9 renderer outright.
    if (data->adapter >= IDirect3D9_GetAdapterCount(data->d3d)) {
